        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                for cell in self.cells:
                    ...
                    if (
                        self.selected_cell
                        and cell != self.selected_cell
                        and cell.is_in_area(event.pos, self.offset)
                        and cell not in self.selected_cell.connections
                    ):
                        # LIMIT: max 2 połączenia, HEX = 3
                        max_conns = 3 if self.selected_cell.type == "hex" else 2
                        if len(self.selected_cell.connections) >= max_conns:
                            self.logger.info("Osiągnięto limit połączeń dla tej komórki.")
                            self.selected_cell = None
                            return

                        self.animating_connections.append(AnimatedConnection(self.selected_cell, cell))
                        self.selected_cell = None
                ...
                if self.dragging and self.selected_cell:
                    for cell in self.cells:
                        if cell.is_in_area(event.pos, self.offset):
                            if cell != self.selected_cell and cell not in self.selected_cell.connections:
                                max_conns = 3 if self.selected_cell.type == "hex" else 2
                                if len(self.selected_cell.connections) >= max_conns:
                                    self.logger.info("Osiągnięto limit połączeń dla tej komórki.")
                                    self.selected_cell = None
    ...
    def draw(self, screen: pygame.Surface):
        ...
        if self.dragging and self.selected_cell:
            pygame.draw.line(screen, colors.GREEN,
                            (self.selected_cell.x + self.offset[0], self.selected_cell.y + self.offset[1]),
        ...
        if self.selected_cell:
            ...
            for cell in self.cells:
                if cell != self.selected_cell and cell not in self.selected_cell.connections:
                    ...
                    if self.selected_cell.owner == cell.owner and self.selected_cell.type == "defence" or self.selected_cell.owner != cell.owner and self.selected_cell.type == "attack":
            ...
            sel_x = self.selected_cell.x + ox
            sel_y = self.selected_cell.y + oy
            pygame.draw.circle(screen, (255, 255, 0), (sel_x, sel_y), self.selected_cell.radius + 6, 4)
